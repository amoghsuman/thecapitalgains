...
export default async function CourseDetailPage({
  ...
  const trackLabel = LEARNING_PATHS.find((p) => p.value === course.learningPath)?.title ?? course.learningPath ?? "—";
    ...
    <div className="min-h-screen font-sans pb-20">
      ...
      <header className="pt-32 pb-16">
        <div className="site-container grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          {/* Left: back link, badges, title, description, topics, stats */}
          <div className="lg:col-span-2">
            ...
            {course.topics?.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-6">
                {course.topics.map((topic: string) => (
                  <span
                    key={topic}
                    className="inline-flex items-center border border-hairline text-ink-dim text-[12px] font-medium px-3 py-1 rounded-full"
                  >
                    {topic}
                  </span>
                ))}
            ...
            <div className="flex gap-12 mt-12 pt-12 border-t border-hairline">
              {[
                { label: "MODULES", val: chapterCount },
          ...
          {/* Right: What You'll Master (compact) + enrollment card, sticky on
              desktop; on mobile this whole column just falls into normal
              flow below the left column (grid-cols-1 stacks children in DOM
              order — no separate mobile-specific markup needed). */}
          <div className="lg:col-span-1">
            <div className="lg:sticky lg:top-32 space-y-6">
                  ...
                  <div className="space-y-3">
                    {course.whatYouLearn.map((point: string) => (
                      <div key={point} className="flex gap-3">
                        <div className="w-4 h-4 rounded-full bg-emerald-50 text-emerald-600 flex-shrink-0 flex items-center justify-center mt-0.5">
                          <svg width="10" height="10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path d="M5 13l4 4L19 7" /></svg>
                        </div>
                        <span className="text-[13px] text-ink-dim leading-snug font-medium">{point}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="bg-panel border border-hairline rounded-3xl p-8 shadow-2xl">
                <div className="mb-8">
      ...
      {/* ── CURRICULUM ── */}
      <main className="site-container mt-4">
        <section>
              ...
              <div key={chapter.title} className="bg-panel border border-hairline rounded-2xl overflow-hidden shadow-sm hover:border-forest/30 transition-colors">
                ...
                <div className="divide-y divide-hairline">
                  {chapter.lessons?.map((lesson: any) => {
                      ...
                      <>
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center">
                            <span className={`text-[14px] font-semibold ${lessonAccessible ? "text-ink group-hover:text-forest" : "text-ink-dim"} transition-colors`}>
                              {lesson.title}
                            </span>
                            {lesson.duration && (
                    ...
                    return lessonAccessible && lesson.slug ? (
                      <Link
                        key={lesson.title}
