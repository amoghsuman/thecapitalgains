  ...
  return (
    <div className="bg-[#FFFFFF] min-h-screen overflow-x-hidden">
      ...
      <section className="max-w-3xl mx-auto px-8 pt-16 pb-10">
        ...
        <p className="text-[17px] text-[#4B3F6B] leading-relaxed">
      ...
      <section className="max-w-3xl mx-auto px-8 mt-8">
        <div className="flex flex-col gap-5">
          <p className="text-[16px] text-[#4B3F6B] leading-relaxed">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {principles.map((p) => (
            ...
            >
              ...
              <div className="text-[18px] text-[#1C0F3F] mb-2 leading-snug">
                {p.title}
              </div>
              <p className="text-[13px] text-[#4B3F6B] leading-relaxed">{p.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-8 mt-12">
        <div className="bg-[#F5F3FF] rounded-2xl px-8 py-8 grid grid-cols-2 sm:grid-cols-4 gap-6">
          {stats.map((s) => (
            <div key={s.label} className="flex flex-col items-center text-center">
              <span className="font-mono text-[28px] font-medium text-[#1C0F3F] leading-none mb-1">
                {s.num}
              </span>
              <span className="text-[12px] text-[#8B7BAB]">{s.label}</span>
            </div>
          ))}
        </div>
      </section>
